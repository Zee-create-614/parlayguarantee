...
def check_spread_cover(pick_team: str, spread: float, home_team: str, away_team: str, home_score: int, away_score: int) -> Optional[bool]:
    """
    Check if a spread pick covered.
    spread: the line FROM THE PICKED TEAM'S perspective (e.g. +4.5 = underdog getting points, -15.8 = favorite giving points)
    """
    if spread is None or home_score is None or away_score is None:
        return None

    pick_norm = normalize_team(pick_team)
    home_norm = normalize_team(home_team)
    
    # Get the picked team's actual score margin
    if pick_norm == home_norm:
        pick_margin = home_score - away_score  # positive = picked team won
    else:
        pick_margin = away_score - home_score  # positive = picked team won
    
    # Pick covers if: actual_margin + spread > 0
    # e.g. Rockets +4.4, won by 4: 4 + 4.4 = 8.4 > 0 → cover
    # e.g. Nuggets +4.5, lost by 1: -1 + 4.5 = 3.5 > 0 → cover  
    # e.g. Cavs -15.8, won by 28: 28 + (-15.8) = 12.2 > 0 → cover
    adjusted = pick_margin + spread
    return adjusted > 0
...
def score_individual_picks(game_date: str, conn: sqlite3.Connection, picks: List[Dict]) -> Dict[str, Any]:
    ...
    for i, pick in enumerate(picks):
        ...
        if spread is not None:
            spread_hit = check_spread_cover(predicted, spread, home, away, home_score, away_score)
