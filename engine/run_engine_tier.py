...
def main():
    ...
    try:
        ...
        # The Odds API key from the requirements
        odds_api_key = "f3c9f91dc369f56dea1b523d3071e1f1"
        
        # Generate tier picks
        results = engine.run(target_date, args.output, odds_api_key)
        
        # Save analyzed_games.json for per-user parlay generation on the website
        analyzed_games = getattr(engine, '_last_analyzed_games', None)
        if analyzed_games:
            ag_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'analyzed_games.json')
            with open(ag_path, 'w', encoding='utf-8') as f:
                json.dump(analyzed_games, f, indent=2, ensure_ascii=False)
            logger.info(f"📊 Saved {len(analyzed_games)} analyzed games to analyzed_games.json")
        
        if results:
            # Apply market-based confidence boosting if available
            logger.info("🎯 Attempting market-based confidence boosting...")
            try:
                # The market booster expects the old format, so we'll skip it for now
                # or adapt it to work with tier format
                logger.info("Market boosting skipped for tier format (needs adaptation)")
                # results = apply_market_boosting(results, sport='NBA')
            except Exception as e:
                logger.warning(f"Market boosting failed: {str(e)}")
                results['_market_boost_error'] = str(e)
            ...
            if total_games == 0:
                ...
                print(f"\n⚠️  No games available for {target_date}")
                print(f"📁 Output saved to: {args.output}")
                print("[READY] Empty picks ready for deployment")
            else:
                ...
                print(f"\n[OK] Success! Generated picks for tier system")
                print(f"[FILE] Output saved to: {args.output}")
                print("[READY] Ready for deployment to the website!")
                ...
                for tier_id in ['single', '2leg', '3leg', '4leg', '5leg', '6leg', '7leg',
                                'spread_single', 'spread_2leg', 'spread_3leg', 'spread_4leg', 'spread_5leg']:
                    if tier_id in tiers:
            ...
            print("\n[WARN] No games available - fallback output created")
            print(f"[FILE] Output saved to: {args.output}")
        ...
        print("[INFO] Check the logs above for details")
