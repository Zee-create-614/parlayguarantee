...
# 7 games ONLY: IND/WAS, HOU/CHA, BKN/CLE, ATL/PHI, DET/NYK, TOR/CHI, PHX/SAS
...
print(f"{len(players)} players on 7-game slate\n")
...
def fill_remaining(locked, cap=50000):
...
# 5-anchor optimization: PG + SG + SF + PF + C, fill G/F/UTIL
best_lineup = None
best_fp = 0
t0 = time.time()
        ...
        for sf in sf_c:
            ...
            for pf in pf_c:
                ...
                for c in c_c:
                    ...
                    locked_sal = pg['salary'] + sg['salary'] + sf['salary'] + pf['salary'] + c['salary']
                    if locked_sal > 50000 - 3*3000: continue
                    
                    locked = {'PG': pg, 'SG': sg, 'SF': sf, 'PF': pf, 'C': c}
                    lu, fp = fill_remaining(locked)
                    count += 1
                    if lu and fp > best_fp:
                        best_fp = fp
                        best_lineup = lu

print(f"Tested {count:,} combos in {time.time()-t0:.1f}s\n")

if best_lineup:
    print(f"=== OPTIMAL 7-GAME SLATE LINEUP ({best_fp:.1f} proj FP) ===\n")
    tot_sal = 0
